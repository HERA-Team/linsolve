...
class Parameter:
    ...
    def sparse_form(self, name, eqnum, prm_order, prefactor, re_im_split=True):
        xs, ys, vals = [], [], []
        # separated into real and imaginary parts iff one of the variables is
        # conjugated with "_"
        if re_im_split:
...
class LinearSolver:
    def __init__(self, data, wgts=None, sparse=False, **kwargs):
        """Set up a linear system of equations of the form 1*a + 2*b + 3*c = 4.

        Args:
            data: Dictionary that maps linear equations, written as valid
                python-interpetable strings that include the variables in question, to
                (complex) numbers or numpy arrarys. Variables with trailing underscores
                '_' are interpreted as complex conjugates.
            wgts: Dictionary that maps equation strings from data to real weights to
                apply to each equation. Weights are treated as 1/sigma^2. All equations
                in the data must have a weight if wgts is not the default, {}, which
                means all 1.0s.
            sparse: Boolean (default False). If True, represents A matrix sparsely
                (though AtA, Aty end up dense). May be faster for certain systems of
                equations.
            **kwargs: keyword arguments of constants (python variables in keys of
                data that are not to be solved for)
    ...
    def get_A(self):
        ...
        # A[xs,ys] += [v * ones for v in vals] # This is broken when a single equation
        # has the same param more than once
    ...
    def get_weighted_data(self):
        """Return y=data*wgt**.5 as 2D vector, regardless of original data/wgt shape."""
    ...
    def _invert_lsqr(self, A, y, rcond):
        """Use np.linalg.lstsq to solve a system of equations.

        Usually the best performer, but for a fully-constrained system, 'solve' can be
        faster.  Also, there are a couple corner cases where lstsq is unstable but pinv
        works for the same rcond. It seems particularly the case for single precision
        matrices.
        """
        ...
        # x = [
        #   np.linalg.lstsq(A[...,k], y[...,k], rcond=rcond)[0]
    ...
    def _invert_lsqr_sparse(self, xs_ys_vals, y, rcond):
        ...
        # this is crazy slow for unknown reasons
        # x = [
        #   scipy.sparse.linalg.lsqr(A[k], y[...,k], atol=rcond, btol=rcond)[0]
        #   for k in range(y.shape[-1])
        # ]
    ...
    def _invert_pinv_shared(self, A, y, rcond):
        ...
        # the following is slow for small matrices
    ...
    def _invert_solve(self, A, y, rcond):
        ...
        singular matrices).  Can by ~1.5x faster that lstsq for this case. 'rcond' is
        unused, but passed as an argument to match the interface of other _invert
        methods.
        """
    ...
    def _invert_solve_sparse(self, xs_ys_vals, y, rcond):
        """Use linalg.solve to solve a fully constrained (non-degenerate) system.

        Tends to be ~3x slower than lsqr for sparse matrices.  'rcond' is unused, but
        passed as an argument to match the interface of other _invert methods.
        """
        AtA, Aty = self._get_AtA_Aty_sparse(xs_ys_vals, y)
        # AtA and Aty don't end up being that sparse, usually
    ...
    def solve(self, rcond=None, mode="default"):
        ...
        Args:
            rcond: cutoff ratio for singular values useed in numpy.linalg.lstsq,
                numpy.linalg.pinv, or (if sparse) as atol and btol in
            ...
            mode: 'default', 'lsqr', 'pinv', or 'solve', selects which inverter to use,
                unless all equations share the same A matrix, in which case pinv is
                always used`.
                ...
                'pinv': uses numpy.linalg.pinv to perform a pseudo-inverse and then
                    solves. Can sometimes be more numerically stable (but slower) than
                    'lsqr'.
                All of these modes are superceded if the same system of equations
                applies to all datapoints in an array.  In this case, a inverse-based
                method is used so that the inverted matrix can be re-used to solve all
                array indices.
    ...
    def eval(self, sol, keys=None):
        """Returns a dictionary evaluating data keys to the current values.
    ...
    def chisq(self, sol, data=None, wgts=None):
        ...
        Weights are treated as 1/sigma^2.
        wgts = {} means sigma = 1. Default uses the stored data and weights unless
...
class LogProductSolver:
    def __init__(self, data, wgts=None, sparse=False, **kwargs):
        """Set up a nonlinear system of equations of the form a*b = 1.0 to linearize
        via logarithm.

        Args:
            data: Dictionary that maps nonlinear product equations, written as valid
                python-interpetable strings that include the variables in question, to
                (complex) numbers or numpy arrarys. Variables with trailing underscores
                '_' are interpreted as complex conjugates (e.g. x*y_ parses as
                x * y.conj()).
            wgts: Dictionary that maps equation strings from data to real weights to
                apply to each equation. Weights are treated as 1/sigma^2. All equations
                in the data must have a weight if wgts is not the default, {}, which
            ...
            sparse: Boolean (default False). If True, represents A matrix sparsely
                (though AtA, Aty end up dense). May be faster for certain systems of
                equations.
            **kwargs: keyword arguments of constants (python variables in keys of data
                that are not to be solved for)
...
class LinProductSolver:
    def __init__(self, data, sol0, wgts=None, sparse=False, **kwargs):
        """Set up a nonlinear system of equations of the form a*b + c*d = 1.0
        to linearize via Taylor expansion and solve iteratively using the
        ...
        Args:
            data: Dictionary that maps nonlinear product equations, written as valid
                ...
                '_' are interpreted as complex conjugates (e.g. x*y_ parses as
                x * y.conj()).
            sol0: Dictionary mapping all variables (as keyword strings) to their
                starting guess values. This is the point that is Taylor expanded around,
                so it must be relatively close to the true chi^2 minimizing solution.
                In the same format as that produced by linsolve.LogProductSolver.solve()
                or linsolve.LinProductSolver.solve().
            wgts: Dictionary that maps equation strings from data to real weights to
                apply to each equation. Weights are treated as 1/sigma^2. All equations
                in the data must have a weight if wgts is not the default, {}, which
                means all 1.0s.
    ...
    def gen_taylors(self, keys=None):
        """Parse terms, perform taylor expansion, and map eqn keys to expansion keys."""
        if keys is None:
            keys = self.keys
    ...
    def build_solver(self, sol0):
        ...
        # Now make sure every taylor equation has every relevant constant, even if they
        # don't appear in the derivative terms.
    ...
    def _update_solver(self, sol):
        """Update all constants in the internal LinearSolver and its LinearEquations
    ...
    def eval(self, sol, keys=None):
        """Returns a dictionary evaluating data keys to the current values given sol
        and consts. Uses the stored data object unless otherwise specified."""
        if type(keys) is str:
            keys = [keys]
        elif type(keys) is dict:
            keys = list(keys.keys())
    ...
    def chisq(self, sol, data=None, wgts=None):
        ...
        wgts = {} means sigma = 1. Uses the stored data and weights unless otherwise
        overwritten.
        """
        if data is None:
            data = self.data
    ...
    def solve_iteratively(
        ...
        """Repeatedly solves and updates linsolve until convergence or maxiter is
        reached. Returns a meta object containing the number of iterations, chisq, and
        convergence criterion.

        Args:
            conv_crit: A convergence criterion below which to stop iterating.
                Converegence is measured L2-norm of the change in the solution of all
                the variables divided by the L2-norm of the solution itself.
                Default: None (resolves to machine precision for inferred dtype)
            maxiter: An integer maximum number of iterations to perform before quitting.
                Default 50.
            mode: 'default', 'lsqr', 'pinv', or 'solve', selects which inverter to use,
                unless all equations share the same A matrix, in which case pinv is
                always used.
